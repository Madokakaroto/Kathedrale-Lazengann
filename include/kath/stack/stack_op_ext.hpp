...
namespace kath
{
    ...
    template <typename T, typename = void>
    struct has_key_map_type : std::false_type {};

    template <typename T>
    struct has_key_map_type<T, std::void_t<
        typename T::key_type,
        typename T::mapped_type
    >> : std::true_type {};

    template <typename T>
    inline constexpr bool has_key_map_type_v = has_key_map_type<T>::value;
    ...
    using is_associative_container = meta_and<
        has_key_map_type<T>, is_container<T>
    >;
...
namespace kath { namespace ext
{
    ...
    struct manipulate_type<T, std::enable_if_t<is_associative_container_v<T>>>
    {
        ...
        static auto stack_push(lua_State* L, C&& c)
            -> std::enable_if_t<std::is_same_v<std::remove_cv_t<std::remove_reference_t<C>>, type>>
        {
            using forward_key_type = detail::forward_type_t<C, key_type>;
            using forward_mapped_type = detail::forward_type_t<C, mapped_type>;

            ::lua_createtable(L, 0, static_cast<int>(c.size()));
            for(auto&& elem : c)
            {
                kath::set_table(L, std::forward<forward_key_type>(elem.first), 
                    std::forward<forward_mapped_type>(elem.second));
            }
        }
