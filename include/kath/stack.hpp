...
namespace kath
{
	...
	{
		return { ::lua_getglobal(L, key.c_str()) };
	}
...
namespace kath
{
	...
	inline static auto fetch_field(lua_State* L, Key const& key, int index = -1)
		-> std::enable_if_t<has_c_str_v<Key>, basic_type>
	{
		return { ::lua_getfield(L, index, key.c_str()) };
	}
	...
	{
		return { ::lua_geti(L, index, static_cast<lua_Integer>(key)) };
	}
}
