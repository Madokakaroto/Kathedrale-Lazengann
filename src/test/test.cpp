...
#define BOOST_TEST_MODULE "kath.test.mpl"
...
BOOST_AUTO_TEST_CASE(BooleanMetaFunction)
{
	...
	BOOST_CHECK((std::is_same_v<std::invoke_result_t<foo, void*>, bool>));

	auto r = kath::is_lua_cfunctor<foo&>::value;
	kath::is_lua_cfunctor<bar>::value;
}